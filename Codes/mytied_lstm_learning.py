...
with tf.device("/device:GPU:1"):
	...
	for i in range(batch_size):
		...
		if((i>0)|(j>0)):
                        LSTM_ONLY.load_weights("prev_weights.h5")
                if(np.isin(index[i],only_train)==False):#val also exist
                        ...
                        a=LSTM_ONLY.fit(x=X_train[i], y=X_train[i], epochs=1, batch_size=batch, shuffle=True, validation_data=(val[i],val[i]))
                        LSTM_ONLY.save_weights("prev_weights.h5")
                else:
                        batch=int(round(len_num[index[i]]))
                        a=LSTM_ONLY.fit(x=X_train[i],y=X_train[i], epochs=1, batch_size=batch, shuffle=True)
                        LSTM_ONLY.save_weights("prev_weights.h5")
                print(LSTM_ONLY.summary())
        model_name="mytied_encoder_lstmonly_hidden100_"+str(j)+".h5"
        LSTM_ONLY.save_weights(model_name)
        print("*******\n")
