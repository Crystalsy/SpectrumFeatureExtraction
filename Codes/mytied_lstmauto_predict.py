...
with tf.device("/device:GPU:1"):
    ...
    X_hat_test=[]
    for j in range(30):
	model_name="mytied_encoder_lstmauto_hidden100_"+str(j)+".h5"
	for i in range(batch_size):
	    K.clear_session()
	    with tf.device("/device:GPU:1"):
		...
		LSTM_AE.load_weights(model_name)
                if(np.isin(index[i],only_train)==False):#val also exist
                        train_result.append(LSTM_AE.predict(X_train[i],batch_size=int(round(len_num[index[i]]))))
                        val_result.append(LSTM_AE.predict(val[i],batch_size=int(round(test_num[index[i]]))))
                        X_hat_test.append(LSTM_AE.predict(X_test[i],batch_size=int(round(test_num[index[i]]))))
                else:
                        batch=int(round(len_num[index[i]]))
                        train_result.append(LSTM_AE.predict(X_train[i],batch_size=batch))
                        val_result.append(np.zeros((1,)))
                        X_hat_test.append(np.zeros((1,)))
        print("____________\n")
        print(time.time() - start_time)

f1=open("mytied_encoder_lstmauto_han_unit100_batch100_pred_train.txt",'w')
f2=open("mytied_encoder_lstmauto_han_unit100_batch100_pred_val.txt",'w')
f3=open("mytied_encoder_lstmauto_han_unit100_batch100_pred_test.txt",'w')
