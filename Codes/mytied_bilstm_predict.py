...
with tf.device("/device:GPU:1"):
	...
	for i in range(batch_size):
		...
		BILSTM.load_weights(model_name)
		if(np.isin(index[i],only_train)==False):#val also exist
                        train_result.append(BILSTM.predict(X_train[i],batch_size=int(round(len_num[index[i]]))))
                        val_result.append(BILSTM.predict(val[i],batch_size=int(round(test_num[index[i]]))))
                        X_hat_test.append(BILSTM.predict(X_test[i],batch_size=int(round(test_num[index[i]]))))
                else:
                        batch=int(round(len_num[index[i]]))
                        train_result.append(BILSTM.predict(X_train[i],batch_size=batch))
                        val_result.append(np.zeros((1,)))
                        X_hat_test.append(np.zeros((1,)))
        print("____________\n")
        print(time.time() - start_time)

f1=open("mytied_encoder_bilstmonly_han_unit100_batch100_pred_train.txt",'w')
f2=open("mytied_encoder_bilstmonly_han_unit100_batch100_pred_val.txt",'w')
f3=open("mytied_encoder_bilstmonly_han_unit100_batch100_pred_test.txt",'w')
